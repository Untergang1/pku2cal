...
当前收录的是**校本部 2026–2027 第一学期校历**，默认预设和仓库附带的配置均选择**软微作息**。打开 `config/calendar.json`，按自己的课程修改 `timetable` 字段，再运行 `npm run status` 核对：
...
一份课表统一使用一套作息。**切换作息不会改变学期日期、假期或补课安排**；其他学期、校区或学院的校历差异需要另行核对和调整，见[校历配置](docs/advanced.md#校历配置)。如果配置文件不存在，可运行 `npm run setup` 创建使用软微作息的默认配置；该命令不会覆盖已有的不同配置。
