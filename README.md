...
| GitHub Pages | 固定的日历订阅地址 | 手动发布，等待日历应用刷新 | 课表公开可访问，有一定暴露风险，详见下文 |
...
内置校本部（`pku-main`）和软件与微电子学院（`pku-ss`）两套作息，默认使用软微作息。修改 `config/calendar.json` 中的 `timetable` 即可切换；软微第 8 节映射为 **13:00–13:50**。
...
默认拉取现有配置对应的学期；配置不存在时，按当前日期选择学期。覆盖前会备份原配置，日期及手工停补课修订会整体替换。可用 `npm run status` 核对学期，其他参数与完整时间表见[校历配置](docs/advanced.md#校历配置)。
...
任何知道订阅地址中随机令牌的人都可以访问课表，请保密完整地址。详细风险见[进阶文档](docs/advanced.md#pages-隐私风险)。
