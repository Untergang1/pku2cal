...
- [软微时间表](config/timetables/pku-ss.json)：已按用户观察手动维护 12 个节次；属于实际使用修正，尚无已核实的官方节次表。

时间表文件包含 `label`（显示名称）和 `periods` 数组。每节使用 `{ "period": 1, "start": "08:00", "end": "08:50" }` 的结构（这里只演示本部第一节的格式，不是软微时间）。编号为 1–30 的整数，时间按 `Asia/Shanghai` 解释，须为 `HH:MM`；编号不能重复、开始须早于结束、各节不能重叠，且必须覆盖课程涉及的全部节次。编号不要求按实际时间递增，重叠检查按实际时间进行。

修改日历配置中的 `timetable` 即可切换；当前 `config/calendar.json` 已选择软微。运行 `npm run status` 检查所选时间表和学期；其他日历文件可用 `npm run status -- --config data/calendar.json`。所选文件缺失、内容无效或为空时会报错，不会回退本部。未选中的时间表不读取，软微留空不影响本部使用。课程的节次范围先按编号选取全部节次，再按实际时间排序，以最早开始和最晚结束生成一个日历事件；保留范围内的课间间隔。节次编号不重排，事件 UID 仍使用原始节次范围。课程需要的节次缺失或范围无效时停止生成并报 `schedule:periods`。

**软微第 8 节的手动修正**：用户观察到选课系统的课时存在问题，因此将系统第 8 节映射为 **13:00–13:50**；第 5–7 节仍分别从 14:00、15:00、16:00 开始。按用户确认的实际安排，**5–7 节为 14:00–16:50，5–8 节为 13:00–16:50**：后一范围内的实际顺序是第 8、5、6、7 节，无需重新编号。未来系统课时或实际安排变化时，重新核对后编辑软微时间表即可。该说明来源于用户观察，不代表学院官方作息声明。
