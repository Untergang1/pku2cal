...
修改日历配置中的 `timetable` 即可切换；当前 `config/calendar.json` 已选择软微。运行 `npm run status` 检查所选时间表和学期；其他日历文件可用 `npm run status -- --config data/calendar.json`。所选文件缺失、内容无效或为空时会报错，不会回退本部。未选中的时间表不读取，软微留空不影响本部使用。生成时发现课程需要的节次缺失，或一段连续节次中实际时间倒序（例如当前软微的 7–8 节），会停止生成并报 `schedule:periods`，不猜测重排或合并。

**软微第 8 节的手动修正**：用户观察到选课系统的课时存在问题，因此将系统第 8 节映射为 **13:00–13:50**；第 5–7 节仍分别从 14:00、15:00、16:00 开始。这是有意保留的节次映射补丁，不应仅因第 8 节时间更早而改成 17:00。未来系统课时或实际安排变化时，重新核对后编辑软微时间表中 `period: 8` 的起止时间即可恢复对应关系；无需修改代码。该说明来源于用户观察，不代表学院官方作息声明。
